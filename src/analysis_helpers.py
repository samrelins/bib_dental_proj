...
def return_cramersv_heatmap(dataf, skip_cols, title=""):
    cat_cols = [col for col in dataf.columns
                if dataf[col].dtype.name in ["category", "object"]
                and col not in skip_cols]
    platoon_cramers_v = build_cramersv_df(dataf, cat_cols)
