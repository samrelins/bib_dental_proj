...
def build_epi_ga_data(data_dir, include_ids=False):
    ...
    ga_data["dgaGATotal"] = (ga_data.dgaGATotal
                             .map(correct_tot_seq_vals)
                             .astype("int"))
    ga_data["dgaGASequence"] = (ga_data.dgaGASequence
                                .map(correct_tot_seq_vals)
                                .astype("int"))
    ...
    # and re-format treatment names in lower case
    tooth_cols = [col for col in merge_data.columns
                  if col[:4] in ["dgaU", "dgaL"]]
    for col in tooth_cols:
        merge_data[col] = (merge_data[col]
                           .fillna("nil_treatment")
    ...
    # remove duplicate entries - seem to have a few kids with duplicate entries
    # that have been interpreted as a first and second anaesthetic
    ga_sequence_2 = ga_data.dgaGASequence == 2
    total_gas_3 = ga_data.dgaGATotal == 3
    duplicate_ga_entries = ga_sequence_2 & total_gas_3
    ga_data = ga_data[~duplicate_ga_entries]

    merge_sequence_2 = merge_data.dgaGASequence == 2
    merge_total_3 = merge_data.dgaGATotal == 3
    duplicate_merged_entries = merge_sequence_2 & merge_total_3
    ...
    rename_cols = {
        ...
        'Totalnoextractions': 'n_extracted',
        'totalnumberofprimaryextractions': 'n_primary_extractions',
        'totalnumberofpermanentextractions': 'n_perm_extractions',
        'AgeMths': 'age_at_ga',
        'ddsdt': 'decayed_teeth',
        'ddsmt': 'missing_teeth',
        'ddsft': 'filled_teeth',
