...
while (teeth_string=="y") {
  ...
  # xy = Calibrate(data.points, cal, 10, 0, 10, 0)
  ...
  # pretty sure this isn't needed anymore - kept just in case
  # xy$teeth <- i
  # if (!exists("points_store")) { 
  #   points_store <- xy 
  # } else { 
  #   points_store <- rbind.na(points_store, xy)
  # }
...
data = data.frame(a=1:length(polygons_list)) ### data goes here - currently a placeholder ###
spdf = SpatialPolygonsDataFrame(sps, data)
shapefile(x=spdf, file="~/Desktop/mouth_map", overwrite=TRUE)

# plot spatial dataframe - pretty sure that GIS version of ggplot would do this much better
plot(spdf)
